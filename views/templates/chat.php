<div class="chat">
    <div class="conversation-list">
        ...
        <?php foreach ($conversations as $conversation) : ?>
            <a
                class="conversation-link <?= ($selectedConversationId == $conversation->getOtherUserId()) ? 'active' : '' ?>"
                href="index.php?action=chat&user=<?= $conversation->getOtherUserId() ?>"
            >
                <img
                    class="other-user-avatar"
                    src="<?= htmlspecialchars($conversation->getOtherUserImage()) ?>"
                    alt="Avatar de <?= htmlspecialchars($conversation->getOtherUserNickname()) ?>"
                >
                <div class="conversation-info">
                    <div class="conversation-header">
                        <span class="other-user-nickname">
                            <?= htmlspecialchars($conversation->getOtherUserNickname()) ?>
                        </span>
                        <span class="last-message-time">
                            <?= htmlspecialchars($conversation->getLastMessage()->getMessageDate()) ?>
                        </span>
                    </div>
                    <div class="conversation-last-message">
                        <span>
                            <?= htmlspecialchars(mb_strimwidth($conversation->getLastMessage()->getContent(), 0, 50, '...')) ?>
                        </span>
                    </div>
    ...
    <div class="selected-conversation">
        <div class="other-user-info">
            <img
                class="other-user-avatar"
                src="<?= htmlspecialchars($otherUser->getImage()) ?>"
                alt="Avatar de <?= htmlspecialchars($otherUser->getNickname()) ?>"
            >
